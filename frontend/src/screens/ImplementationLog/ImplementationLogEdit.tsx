...
interface ImplementationLogEditProps {
  record: ImplementationLogRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const ImplementationLogEdit: React.FC<ImplementationLogEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<ImplementationLogRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<ImplementationLogRecord>) =>
      dataProvider.update('ImplementationLog', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'ImplementationLog', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit ImplementationLog</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  <LookupSelect label="Phase *" value={String(form.phase ?? '')} onChange={(v) => setField('phase', v ? Number(v) : null)} searchable={false} options={[{ label: 'RoutePlanning', value: '0' }, { label: 'System', value: '1' }, { label: 'Operations', value: '2' }]} />
                </div>
                <div>
                  <TkInput mode="text" label="Notes" value={String(form.notes ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('notes', v))(e.detail)} />
                </div>
                <div>
                  <TkDatepicker label="Completed At" value={String(form.completedAt ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('completedAt', v))(e.detail)} />
                </div>
                <div>
                  <LookupSelect label="Değişiklik Talebi *" resource="ChangeRequest" value={String(form.changeRequestId ?? '')} onChange={(v) => setField('changeRequestId', v)} displayField="title" />
                </div>
            </div>
          </div>
