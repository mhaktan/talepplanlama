...
interface RequestTypeEditProps {
  record: RequestTypeRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const RequestTypeEdit: React.FC<RequestTypeEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<RequestTypeRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<RequestTypeRecord>) =>
      dataProvider.update('RequestType', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'RequestType', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit RequestType</h2>
